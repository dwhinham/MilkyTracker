...
void Tracker::checkForChanges(ModuleEditor* moduleEditor/* = NULL*/, std::function<void(bool)> onCompletion)
{
	...
	{
		PPQuitSaveAlert *quitSaveAlertDialog = new PPQuitSaveAlert(screen);
		
		quitSaveAlertDialog->runModal([&](PPModalDialog::ReturnCodes code, PPString)
		{
			if (code == PPQuitSaveAlert::ReturnCodeOK)
			{
				PPSavePanel *savePanel = new PPSavePanel(screen, "Save Extended Module", moduleEditor->getModuleFileName());
				savePanel->addExtension("xm","Fasttracker 2 Module");
				
				savePanel->runModal([&](PPModalDialog::ReturnCodes code, PPString filename)
				{
					if (code == PPSavePanel::ReturnCodeOK)
					{
						if (filename)
						{
							moduleEditor->saveSong(filename);
						}
					}
				});
...
void Tracker::checkForChangesOpenModule(std::function<void(bool)> onCompletion)
{
...
void Tracker::shutDown(std::function<void(bool shutDown)> onShutDown)
{
	int numEditors = tabManager->getNumTabs();
	int editorsNotified = 0;
	
	for(int i = 0; i < numEditors; ++ i)
	{
		ModuleEditor *moduleEditor = tabManager->getModuleEditorFromTabIndex(editorsNotified);
		
		checkForChanges(moduleEditor, [&] (bool shouldClose)
		{
			if(shouldClose && ++ editorsNotified == numEditors)
			{
